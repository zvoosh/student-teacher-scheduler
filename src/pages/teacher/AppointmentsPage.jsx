...
const AppointmentsPage = () => {
  ...
  useEffect(() => {
    if (data) setFilteredData(data.filter((item) => item.approved == true && item.teacherId == loggedinuser.uid));
  }, [data]);
  ...
  const onSearch = (value) => {
    ...
    const preparedData = data.filter((appointment) => {
      if (appointment.approved == true && appointment.teacherId == loggedinuser.uid) {
        ...
        const dateMatch = dayjs(appointment.dateTime._seconds * 1000)
          ...
          .toLowerCase()
          .includes(value.toLowerCase());
        const timeMatch = dayjs(appointment.dateTime._seconds * 1000)
          .format("HH:mm")
          .toLowerCase()
          .includes(value.toLowerCase());

        return studentFullNameMatch || dateMatch || timeMatch;
      }
      return;
    });
    ...
    <div className="w-full !p-5 2xl:!p-0 xl:w-3/5 2xl:w-3/5 h-full">
      ...
      <div className="h-fit !py-5 2xl:!py-10">
        <div className="w-full text-center !mb-5 2xl:!mb-10">
          <h1 className="text-xl 2xl:text-4xl">Appointments</h1>
        </div>
        <div className="flex flex-col items-center justify-center !my-10 2xl:!my-20">
          <div className="w-full flex justify-start !mb-10">
            {data && data.filter((item)=> item.approved == true).length > 0 && (
              <Search
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col rounded-lg">
                      <div className="flex justify-between items-center text-lg font-bold bg-purple-600 !p-2 rounded-lg">
